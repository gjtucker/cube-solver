...
- **Fast 4×4 solving in the browser** — 228 KB of compressed lookup tables load in ~50 ms; the big exact-search pruning table (57 MB, 239.5M entries at 2 bits) is built on-device inside each Web Worker and prewarmed in the background when you enter 4×4 mode. A fewest-moves solve then takes about a second at ~46 moves, or ~44 with "Search harder", without freezing the page.
